...
The URLs are stable (always the most recent green build); it's a **pre-release**, so
APIs may change until the first tagged release. Or build from source:

[source,bash]
----
git clone https://github.com/alexmond/jvmlens && cd jvmlens
./mvnw -q clean package
#   target/jvmlens.jar                  <- CLI + MCP server (analyze/profile/watch/mcp)
...
# inline from a JMH benchmark (put jvmlens-<version>-jmh.jar on the classpath):
