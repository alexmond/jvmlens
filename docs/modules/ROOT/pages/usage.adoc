...
no extra instrumentation; degrades to nothing if Micrometer is absent), and `history` (see
*Long-running monitor* below). The `messaging` and `cache` dimensions aggregate by
`Class.method` operation and render top operations by total time. The agent jar is the
separate
