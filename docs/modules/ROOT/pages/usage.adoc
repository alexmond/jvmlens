...
## Totals
- Exec samples: 685 → 235 (-450, -66%)
- GC pause (ms): 40 → 1550 (+1510, +3775%)
## Hot paths (sample share)
- `GoFmt.floatString` — 50%→8% (▼ 42pp)
- `Cache.get` — NEW 40%
- `ListNode.iterator` — GONE (was 16%)
----

It diffs the totals, hot paths, allocation sites, locks, and every extended section by share
(▲/▼ percentage-point change, plus `NEW` / `GONE`), ranked by the size of the change; rows
that barely moved are dropped. `-a`/`-x` scoping applies to both recordings.
