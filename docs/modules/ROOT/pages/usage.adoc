...
prints the summary inline when the trial ends. Put `jvmlens-<version>-jmh.jar` (a tiny
engine+profiler jar, no Spring/picocli/jmh) on the benchmark's classpath and:

[source,bash]
----
java -cp benchmarks.jar:jvmlens-<version>-jmh.jar org.openjdk.jmh.Main \
  -prof "org.alexmond.jvmlens.jmh.JvmlensProfiler:appPackage=com.example;report=cpu"
----

`appPackage` (`+`-separate several) scopes to your code; `report` focuses the summary. The
