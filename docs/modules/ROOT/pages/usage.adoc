...
For a JVM deployed on another host, `profile` and `watch` take `--jmx` instead of a
local `<pid>` and drive JFR over a remote JMX connection — no agent to install, just the
platform FlightRecorder MXBean.

**Start command** — the remote JVM must be launched with JMX remote enabled:
...
Then query it remotely (full `service:jmx:...` URL or a `host:port` shorthand):

[source,bash]
----
java -jar target/jvmlens.jar profile --jmx my-server:7091 -d 30
java -jar target/jvmlens.jar watch   --jmx my-server:7091 --on-gc-ms 200
----

NOTE: the example disables auth/TLS for brevity; secure the JMX endpoint (auth, TLS, or
an SSH tunnel) for anything beyond a trusted network.
