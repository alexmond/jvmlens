...
Options (comma-separated `key=value`): `out` (summary file), `interval` (seconds between
summaries), `settings` (JFR config, default `profile`), and `snapshot` (see below). The
agent jar is the separate `jvmlens-agent.jar` artifact (engine + agent + a relocated
ByteBuddy); it can also be attached dynamically via the `Agent-Class` entry.
