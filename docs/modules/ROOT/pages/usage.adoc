...
* **Source-line anchoring** — a hot-path row's teaser lists the top leaves *where time
actually goes* with counts, each carrying its source line (`com.example.Svc.compute:88
30/168`); allocation sites carry the allocation call-site's line (`:120 · byte[] 4.2 GB`).
The locator turns "diagnose → find the spot → edit" into one step for a coding agent.
* **Leaf confidence** — when no single leaf holds more than ~20% of a path's samples the
teaser is flagged `⚠ diffuse`, so you don't chase a frame that isn't where the cost is.
* **Allocation confidence** — a short recording yields few allocation samples; a
`⚠ Only N allocation samples` caveat means the per-site **byte shares** are noisy (the
**total** bytes stay reliable).
* **Escape-analysis caveat** — an allocation site dominated by a boxed primitive or a captured
lambda is tagged `⚠ … may be scalar-replaced (escape analysis)`: JFR samples the allocation, but
C2 can eliminate a non-escaping one in steady state, so the sampled bytes may be a *false lever*.
Verify any candidate fix with JMH `-prof gc` (exact bytes/op) before acting.
...
When a fix **extracts a helper method**, one allocation drop splits across two rows (the caller
falls, a `NEW` callee appears) — which can read like a regression at a glance. The diff adds an
**Allocation by type (rollup)** block that sums sites by their declaring type, so the net is one
line (e.g. `GoFmt.* — 7.6 GB → 5.8 GB (▼ 24%) [2 methods]`); it shows only types with ≥2
contributing methods and a real net change.
...
When a run holds **many benchmark methods** (a whole `FeatureBenchmark` directory), the merged
summary blends them. Add `--per-recording` for a `## Per-recording breakdown` section that lists
each `.jfr` with its execution-sample count and dominant hot paths — so you see *which* recording
a merged hot path came from without a second single-subdir `analyze` pass. Colliding `profile.jfr`
names are disambiguated by their parent directory. It's multi-file only (a single recording is
already its own breakdown).

[source,bash]
----
java -jar jvmlens.jar analyze /tmp/run-after -a com.example --per-recording
# ## Per-recording breakdown [sampled]
# - `printf-Throughput/profile.jfr` — 32% (489 samples)  (PrintfFunction.lambda$printf$0 55% · extractVerbs 29%)
# - `sprig-Throughput/profile.jfr`  — 20% (300 samples)  (SprigFunctions.get 41% · …)
----
...
unknown option key is a hard error with a did-you-mean (no silent misconfiguration). Socket
I/O events are dropped by default (on a JMH fork the only socket traffic is the harness control
socket — a rotating `localhost:<port>` that would otherwise show up as External I/O and skew the
correlation); pass `socketio=true` to keep them. If the run also enables JMH's GC profiler
(`-prof gc`), the summary is headed with JMH's *exact* `gc.alloc.rate.norm` (bytes/op) next to
jvmlens's *sampled* per-site bytes, so the measured rate and the attribution sit together. The
...
Add JMH's GC profiler (`-prof gc`) to both runs and the `baseline=` run prints a **measured
allocation A/B verdict** above the sampled diff — JMH's *exact* `gc.alloc.rate.norm` before→after
with a significance call (the change is `SIGNIFICANT` only if the relative Δ exceeds the combined
noise band **and** the confidence intervals don't overlap), and a ⚠ when the **sampled** total Δ
disagrees with the **measured** one (JIT elision / sampling redistribution). Use `-f 2`+ so the
cross-fork variance is real — a single fork badly understates it. This gates the optimize loop on
*measured* significance instead of a sampled delta that may be a phantom.

The same verdict adds a **dispersion** line: a genuine structural allocation removal doesn't just
lower the mean, it eliminates the GC-sampling variance that *was* that allocation, so the cross-fork
band collapses (`±17,200 → ±35 /op, variance collapsed ~500×` is a strong real-win signal the mean
alone can't give). And once bytes/op goes **near-deterministic** it prints a **stop signal** —
further allocation tuning has diminishing returns; the residual is intrinsic floor, pivot off
allocation. The diff also hedges a **CPU** hot-path `▲` row whose absolute samples rose while the
total exec samples *fell* (`possible sampling redistribution — total samples fell N%`), so a share
that climbed only because the rest of the profile shrank isn't misread as a regression.

A **measured throughput A/B** verdict sits next to the allocation one — the CPU analog — gating on
JMH's exact primary score (ops/s, us/op, …) with the same significance test. Its job is to stop a
sampled hot-path swing being read as a speedup: if a hot-path share moved a lot but wall-clock
throughput is *flat*, it says so explicitly (`the sampled hot-path share moved (executeArguments
37%→18%) but throughput is flat — a CPU-share shift is not a speedup; the win is in allocation/GC`).
Because the inline `baseline=` matches a single benchmark method, a baseline JFR recorded for a
*different* method is detected and the measured verdict is **skipped with a warning** rather than
emitting confident wrong numbers — record a per-benchmark baseline.

The CPU hot-path diff applies the same redistribution hedge as allocation: a `▲` row whose absolute
samples rose while the total *fell*, rose while the total stayed *~flat*, or **outpaced a
modestly-rising total** (its *share* climbed) is annotated `(possible sampling redistribution …)`,
because under a fixed-duration capture a now-faster workload runs more iterations — an unchanged
frame then accrues more samples and gains share without its per-op cost changing. Whenever hot-path
shares shift more than the total moved, the diff notes that absolute exec-sample deltas conflate
per-op cost with throughput and points to a fixed-iteration `bench` A/B for a clean per-op
comparison.
...
`AbstractStringBuilder.ensureCapacity` → "presize the buffer", and per-call regex compilation
(`java.util.regex.Pattern.*` leaves, or a `String.replaceAll(String,…)`/`matches`/`split`
source line) → "hoist the `Pattern` to a `static final` field and reuse `Matcher`". It is **off
by default** (the report stays clean data) and every line is tagged `[possible]` — a direction
to investigate, never an assertion.

The **semantic dimensions feed `--hints` too**, each with its own mechanical lever, grounded
in the app call-site the row is anchored to (`· at <Class>:line`): `db` — N+1 → batch
(`WHERE id IN (…)` / `@BatchSize`), `SELECT *` → project columns, and repeated single-row
`insert`/`update` → enable JDBC batching; `web` — a high error-rate endpoint → validate/handle
first; `cache` — a low hit rate → check key cardinality / TTL; `messaging` — a synchronous
per-message send → batch or send async; `mongo` — a repeated `find` → `Filters.in`, repeated
`insertOne` → `insertMany`/`bulkWrite`; `redis` — many single-key reads → pipeline / `MGET`.
...
=== Echo the source line (`--source`, opt-in)

Every hot-leaf and allocation-site row already carries a `file:line` anchor (e.g. `:129`).
Point `--source` at your source root(s) and jvmlens reads each anchored line and appends its
text inline — `floatString:129 ⟶ mantissa.substring(0, dot) + mantissa.substring(dot + 1)` —
so a reader (or coding agent) sees the offending line without opening the file. Pass several
roots comma-separated. It is **off by default**, reads files locally, and degrades silently:
a file it can't find is simply left un-annotated (it never fabricates).

[source,bash]
----
java -jar jvmlens.jar analyze recording.jfr --source src/main/java
----
...
| `messaging` | Top messaging operations (agent Kafka / JMS / RabbitMQ instrumentation; ActiveMQ via JMS).
| `cache` | Top cache operations (agent Spring-Cache instrumentation, with a hit-rate flag).
| `mongo` | Top MongoDB operations (agent sync-driver instrumentation).
| `redis` | Top direct Redis commands (agent Lettuce / Jedis instrumentation).
...
dimensions carry signal — it renders an ordered chain (endpoint → SQL → cache → messaging →
I/O → hot path → lock → GC), each link carrying its source anchor (`@ UserRepo:88`) and a
compact flag (`N+1`, `low hit rate`, `sync send`) so an LLM sees *where* to look. By default
it's co-occurrence, not proof — but when the agent's captured call-path shows a deeper op ran
*inside* an endpoint's handler (the row's `↳ under <Class>` marker matches the endpoint's
anchor class), the note is upgraded to a **✓ Confirmed chain (shared call path)**; otherwise
it degrades to the honest co-occurrence wording (e.g. an offline recording with no agent
call-paths).
...
`-x` scopes the hot-path and allocation-**site** blocks (by the calling frame's
package) and also folds the **Top allocated types** block: any allocated type
whose package matches an exclude (including array types like
`[Lorg.h2.mvstore.Page$PageReference;`) collapses into one accounted
`«excluded types (-x), rolled up»` row, so a test capture's embedded
infrastructure — e.g. an in-process H2's MVStore types — doesn't crowd out your
app's `byte[]` / `String` / domain types. The bytes stay counted, just rolled up
(#128):

[source,bash]
----
java -jar jvmlens.jar analyze -a org.alexmond -x org.h2 recording.jfr
----
...
(HTTP endpoints)* below), `messaging` (time Kafka / JMS / RabbitMQ send + poll/receive/basicPublish;
ActiveMQ via JMS), `cache` (time Spring `Cache` get/put/evict, with a hit-rate flag),
`mongo` (time MongoDB sync-driver ops — see *MongoDB* below), `redis` (time direct Lettuce/Jedis
commands — see *Redis* below), `micrometer` (summarize an existing Micrometer registry —
...
see *Runtime control* below), `control` (a file the agent watches for in-flight
commands), and `scope` (pin the application scope at launch — see below). The `messaging` and `cache` dimensions aggregate by `Class.method` operation and
...
By default the agent attributes application frames heuristically (everything non-JDK and
non-jvmlens). For a headless long-running monitor — a container with no control channel
wired — pin the scope to the target module at launch with `scope`, so `history` / `trend`
attribute to the actual module from the *first* sample instead of guessing:

[source,bash]
----
# app = org.alexmond.unitrack, from sample 1 (no control file needed)
java -javaagent:jvmlens-agent.jar=out=/agent/x.md,history=/agent/x.jsonl,scope=app:org.alexmond.unitrack -jar app.jar
----

Segments are `+`-separated (the arg list is comma-separated, so `+` avoids the clash);
each is `app:<prefix>` or `exclude:<prefix>` (a bare prefix defaults to `app`), e.g.
`scope=app:org.alexmond.unitrack+exclude:org.h2`. This is the launch-time equivalent of the
in-flight `scope app <prefix>` control command below.
...
`enable <dim>` / `disable <dim>` (db/web/messaging/cache/mongo/redis/micrometer/snapshot/deadlock),
...
When the window spans **several JVM lifetimes** — a multi-day monitor across rolling
redeploys — `trend` detects each restart from the large gap it leaves between sample
timestamps (the agent appends on a fixed interval, so a replaced pod leaves a hole far
larger than one interval) and adds a `## Lifecycle` note. Each lifetime's first window is a
cold-start burst (classload, warmup, JIT — a 5–10× outlier that would otherwise skew the
whole-window trend to "rising"), so it is **excluded from the steady-state aggregates**, and
retention is assessed within the **latest** lifetime only (old-object counts reset on
restart). A gapless single-lifetime run is unaffected.
...
call count, average latency, the app call-site it was issued from (`· at <Repo>:line`), and a
shape-aware hint: a high-count low-latency `select` is a *possible N+1* (unless it already uses
`WHERE id IN (…)`, the batch-fetch fix — then it isn't flagged), while a repeated single-row
`insert`/`update` reads as *likely un-batched* (enable JDBC batching):
...
Add `messaging` and the agent times each broker at its **lowest client level** — Kafka
`Producer`/`Consumer` (`send`/`poll`), JMS `MessageProducer`/`MessageConsumer` (`send`/
`receive`, both `jakarta.jms` and `javax.jms`), and RabbitMQ `com.rabbitmq.client.Channel`
(`basicPublish`/`basicGet`) — matched by interface name, so jvmlens needs no broker
dependency. Hooking the client (not the Spring template above it — `KafkaTemplate`/
`JmsTemplate`/`RabbitTemplate`) means a Spring app is instrumented once, never double-counted.
**ActiveMQ** (Classic on `javax.jms`, Artemis on `jakarta.jms`) is covered via the JMS path.
A high-volume per-message publish with material latency is flagged a *synchronous per-message
send* (batch or send async).

=== MongoDB

Add `mongo` and the agent instruments the sync driver's `com.mongodb.client.MongoCollection`
operations (`find`/`aggregate`/`insert*`/`update*`/`delete*`/…, matched by name, no Mongo
dependency) into a `MongoDB operations (by total time)` section, each anchored to its app
call-site. The document analogues of the SQL N+1 levers: a repeated `find`/`aggregate` reads
as a *possible N+1 document fetch* (batch with `Filters.in(…)` / an aggregation), a repeated
single-document `insertOne`/`updateOne`/`deleteOne` as *un-batched* (use `insertMany`/
`bulkWrite`). Note `find`/`aggregate` return lazy iterables, so their *count* is the N+1
signal, not their latency.

[source,bash]
----
java -javaagent:jvmlens-agent.jar=out=/var/log/jvmlens.md,mongo,interval=60 -jar app.jar
----

=== Redis

Add `redis` and the agent times direct Redis commands — Lettuce (`RedisAsyncCommands`, which
the sync API delegates to) and Jedis (`JedisCommands`), matched by name — into a
`Redis commands (by total time)` section, anchored to the app call-site. This is distinct
from the `cache` dimension (which tracks Spring `Cache` hit rate); `redis` is for apps hitting
Redis directly (session stores, rate limiters, queues). A high count of single-key reads
(`get`/`hget`/…) is flagged *possible N+1 round-trips* — pipeline them or use a multi-key
command (`MGET`/`HMGET`).

[source,bash]
----
java -javaagent:jvmlens-agent.jar=out=/var/log/jvmlens.md,redis,interval=60 -jar app.jar
----
...
[TIP]
====
For **forked-test workloads (Maven Surefire/Failsafe)**, prefer live attach
(`jvmlens profile <pid>`) over `-XX:StartFlightRecording=…,dumponexit=true` on the fork's
`argLine`. Surefire reuses and then *kills* its fork rather than exiting it cleanly, so the
`dumponexit` shutdown hook never fires and you get **no `.jfr` at all**. Attaching to the
running fork captures cleanly where the JVM-flag path silently produces nothing.
====
