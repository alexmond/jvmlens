...
Spring `Cache` get/put/evict), and `history` (see *Long-running monitor* below). The
`messaging` and `cache` dimensions aggregate by `Class.method` operation and render top
operations by total time. The agent jar is the separate
