...
samples rose while the total *fell* — or rose while the total stayed *~flat* — is annotated
`(possible sampling redistribution …)`, because under a conserved sample total a frame's rise is a
larger *share*, not necessarily more work. When the total is ~flat the diff also notes that
absolute exec-sample deltas conflate per-op cost with throughput (a fixed-duration capture of a
faster run does more iterations) and points to a fixed-iteration `bench` A/B for a clean per-op
comparison.
