...
summaries), `settings` (JFR config, default `profile`). The agent jar is the separate,
dependency-free `jvmlens-agent.jar` artifact (it can also be attached dynamically via the
`Agent-Class` entry).
