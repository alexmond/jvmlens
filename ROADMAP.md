...
- [ ] **E1 — JFR-native I/O + VT-pinning** (engine-only; ships first): socket/file I/O sections
      and `jdk.VirtualThreadPinned`, new `io`/`pinning` report focuses, history/trend coverage.
- [ ] **E2 — semantic web/db/messaging** via the agent's existing ByteBuddy (E2b SQL+N+1 first,
      then E2a web, E2c messaging, E2d cache); sanitized SQL, opt-in, prod-safe.
- [ ] **E3 — consume existing observability** (OTel / Micrometer / Actuator) instead of re-instrumenting.
- [ ] **E4 — unified rendering + correlation** (slow endpoint → query → GC) + long-run coverage.
