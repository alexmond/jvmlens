...
> an existing `.jfr` or a live JVM (`profile <pid>`), rendered as markdown, JSON, or
> an LLM prompt — or served to an agent over MCP (`jvmlens mcp`). async-profiler
> fidelity and continuous dump-on-trigger are on the roadmap.
