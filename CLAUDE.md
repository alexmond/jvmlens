...
- 2026-06-26 — **lab-leak-scan** — `scripts/lab-scan.sh` + `scripts/lab-deny.txt` (regex denylist of internal DNS, lab hostnames, the registry pull-secret, RFC1918 IPs) fail on **new** lab-internal references; existing debt allowlisted in `scripts/lab-scan-baseline.txt`. A `.githooks/pre-commit` guard (`lab-scan.sh --install-hook`) + a `lab-scan` CI workflow. Why: repos leak lab topology going public — block regressions while #54 scrubs the debt. Promote the denylist to `infra/lab/` for reuse (Forgejo ticket).
