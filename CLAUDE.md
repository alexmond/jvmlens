...
- 2026-06-23 — **async-profiler** — spike confirmed GO: ap-loader (`me.bechberger:ap-loader-all`) runs here (cpu event ok at perf_event_paranoid=2), emits JFR the existing `Summarizer` reads unchanged, and adds native frames (e.g. `libjvm.so.*`). Why: integration is capture→JFR→existing engine; queued as the next big feature.
