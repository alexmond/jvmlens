...
- 2026-07-09 — **agent-real-driver-its** — #146 forked-JVM Testcontainers ITs (mongo+redis) caught two bugs unit/harness tests can't. **redis fired nothing**: Lettuce declares `get`/`set` in `AbstractRedisAsyncCommands`, but the matcher matched only `RedisAsyncCommands` (the impl iface) — ByteBuddy advises only methods a matched type *declares*; broadened to `nameStartsWith("io.lettuce.core.api.async.")`. Also fixed #147's mongo label going bare against the non-public real `MongoCollectionImpl` (resolve via a public supertype). Why: a by-name matcher can match a type that doesn't *declare* the methods.
...
- 2026-07-07 — **rabbitmq-messaging** — `messaging` now also hooks RabbitMQ at the low-level `com.rabbitmq.client.Channel` (`basicPublish`/`basicGet`) — covers native + Spring AMQP, no double-count. Sync-send flag generalized to a `SEND_METHODS` set (`send`/`basicPublish`), broker-agnostic. ActiveMQ deliberately **not** given native support: Classic (`javax.jms`) + Artemis (`jakarta.jms`) already covered via JMS, and Artemis-core would double-count JMS-over-Artemis. Why: hook the lowest client, never the Spring template above it.
- 2026-07-07 — **redis-dimension** — P5b closes P5: opt-in `redis` dimension. `RedisCapture` matches Lettuce `RedisAsyncCommands` + Jedis `JedisCommands` by name (no dep; sync Lettuce is a reflective proxy → target the async impl); `RedisStore` is `OpStore`-backed. `flag`: many single-key reads (`get`/`hget`/…) → possible N+1 round-trips → pipeline/`MGET`. Distinct from `cache` (Spring-Cache hit rate): raw direct-Redis traffic. `codecov.yml` also ignores `*Advice`/`*Capture` glue.
